...
try {
    ...
    switch ($decoded_jwt->rol) {
        case 'docente':
            // Delete if the docente has permission based on the EXISTS condition
            $sql = "
                DELETE opciones_terminales_cursos FROM opciones_terminales_cursos
                INNER JOIN roles_cursos ON roles_cursos.id_curso = opciones_terminales_cursos.id_curso
                WHERE opciones_terminales_cursos.id = ? AND roles_cursos.id_maestro = ?
            ";
            ...
            // Bind the parameters for docente (id, id, decoded_jwt->sub)
        ...
        case 'god':
            ...
            $sql = "
                DELETE FROM opciones_terminales_cursos WHERE id = ?
            ";
            $stmt = $connection->prepare($sql);
            if ($stmt === false) {
                throw new Exception('Prepare statement failed: ' . $connection->error);
    ...
    if ($stmt->affected_rows === 0) {
        echo json_encode([
            'success' => false,
            'message' => 'No row was deleted. You may not have the necessary permissions.',
        ]);
        http_response_code(403);  // Return 403 Forbidden when deletion fails due to permission
    } else {
        echo json_encode([
            'success' => true,
            'message' => 'Opcion Terminal eliminada exitosamente',
        ]);
    }
