...
try {
    ...
    switch ($decoded_jwt->rol) {
        case 'docente':
            ...
            $sql = "
                DELETE coordinaciones_cursos FROM coordinaciones_cursos
                INNER JOIN roles_cursos ON roles_cursos.id_curso = coordinaciones_cursos.id_curso
                WHERE coordinaciones_cursos.id = ? AND roles_cursos.id_maestro = ?
            ";
        ...
        case 'god':
            ...
            $sql = "
                DELETE FROM coordinaciones_cursos WHERE id = ?
            ";
            $stmt = $connection->prepare($sql);
            if ($stmt === false) {
                throw new Exception('Prepare statement failed: ' . $connection->error);
    ...
    if ($stmt->affected_rows === 0) {
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Coordinacion del Curso eliminada exitosamente',
        ]);
    }
